...
fetched_at: "2025-12-05T18:58:28.518Z"
---
...
### [Can you spot the bug?](#can-you-spot-the-bug)

In the imperative example, a bug has been intentionally introduced for didactical purposes. Can you find it? It’s not easy!

Show me the answer!

In the case that the Show button is pressed first, then the Activate button, and finally the Hide button, it doesn’t add the 
...
### [Benefits of the declarative approach](#benefits-of-the-declarative-approach)
...
3.  **Reactive Bindings**: HTML elements are bound to reactive state values using special attributes like 
...
*   ```
    ...
    ```
     ...
     are basic state properties.
*   ```
    visibilityText
    ```
    
     is a derived state that automatically updates when 
...
*   ```
    ...
    ```
     ...
     is an action that modifies the state.
...
1.  When the button is clicked, it triggers the 
...
3.  The Interactivity API detects this change and automatically:
    *   Updates the button’s text content (because of 
    ...
    *   Updates any other parts of the DOM that depend on 
...
There’s no need to create a new array or use the spread operator as you might in other frameworks. The Interactivity API will detect this change and update any parts of the UI that depend on 
...
### [Reactive side effects](#reactive-side-effects)

In addition to automatically updating the UI, the Interactivity API allows you to perform side effects when reactive data changes using directives like 

```
data-wp-watch
```

. Side effects are useful for tasks like logging, making API calls, or updating other parts of your application that aren’t directly tied to the UI.
...
1.  The 
    ...
    ```
     ...
     directive adds a local context with a property 
...
4.  The 
    ...
    ```
     ...
     callback logs the current counter to the console.

This allows you to create declarative side effects that automatically run in response to data changes. Some other use cases for 
