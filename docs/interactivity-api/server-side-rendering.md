...
fetched_at: "2025-12-05T18:58:29.755Z"
---
...
## [Processing the directives on the server](#processing-the-directives-on-the-server)
...
To understand how the Server Directive Processing works, let’s start with an example where a list of fruits is rendered using the 
...
*   **3\. Define the interactive elements using directives**
    ...
    *   The 
        ...
        ```
         ...
         directive activates the interactivity for the DOM element and its children.
    *   The 
        ...
        ```
         ...
         directive is used to render a list of elements. The directive can be used in 
        
        ```
        <template>
        ```
        
         tags, with the value being a reference path to an array stored in the global state or the local context.
    *   The 
        ...
        ```
         ...
         directive sets the inner text of an HTML element. Here, it points to 
...
That’s it! Once you’ve set up your interactive block with 
...
Behind the scenes, WordPress uses the 
...
```
 ...
 directive has generated a 

```
<li>
```

 element for each fruit in the array, and the 

```
data-wp-text
```

 directive has been processed, populating each 
...
## [Manipulating the global state and local context in the client](#manipulating-the-global-state-and-local-context-in-the-client)
...
This new button has a 

```
data-wp-on-async--click
```

 directive that references 
...
## [Initializing the derived state in the server](#initializing-the-derived-state-in-the-server)
...
### [Derived state that can be defined statically](#derived-state-that-can-be-defined-statically)
...
Now, let’s display a special message when there is no fruit. To do this, let’s use a 

```
data-wp-bind--hidden
```

 directive that references a derived state called 
...
Up to this point, everything is fine in the client, and when we press the “Delete all fruits” button, the “No fruits, sorry!” message will be displayed. The problem is that since 

```
state.hasFruits
```

 is not defined on the server, the 
...
To fix this, you must define the initial value of the derived state in the server using 
...
Regardless of the approach, the key point is that the initial value of 

```
state.hasFruits
```

 is now defined on the server. This allows the Server Directive Processing to handle the 

```
data-wp-bind--hidden
```

 directive and modify the HTML markup, adding the 
...
### [Derived state that needs to be defined dynamically](#derived-state-that-needs-to-be-defined-dynamically)
...
Again, up to this point, everything is fine on the client side and the visitor will see the correct emoji displayed for the fruits they have on their shopping list. However, since 

```
state.onShoppingList
```
...
Let’s fix that by adding the initial derived state using 

```
wp_interactivity_state
```

. Remember that this time, the value depends on 
...
## [Serializing other processed values to be consumed on the client](#serializing-other-processed-values-to-be-consumed-on-the-client)
...
But wait, what happens with our 
...
## [Processing directives in classic themes](#processing-directives-in-classic-themes)

Server Directive Processing happens automatically in your interactive blocks as soon as you add 
...
Classic themes can also use the Interactivity API, and if they want to take advantage of the Server Directive Processing (which they should), they can do so through the 
...
```
 ...
 in a template file, you can use 
...
**Important:** You only need to process the directives once. If you’re including an inner template file within another template, ensure that 
